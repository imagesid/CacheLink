...
jlong rocksdb_open_helper(JNIEnv* env, jlong jopt_handle, jstring jdb_path,
  ...
  std::cerr << "[JNI-FORCED] Setting use_direct_reads = true" << std::endl;
  opt->use_direct_reads = true;
  
  std::cerr << "[JNI-FORCED] Setting use_direct_io_for_flush_and_compaction = true" << std::endl;
  opt->use_direct_io_for_flush_and_compaction = true;

  if (opt->statistics == nullptr) {
      std::cerr << "[JNI-FORCED] Creating new DB Statistics object" << std::endl;
...
jlongArray rocksdb_open_helper(
  ...
  // We force these to true to ensure the Baseline cannot use OS Page Cache
  std::cerr << "[JNI-FORCED] Setting Direct I/O flags..." << std::endl;
  opt->use_direct_reads = true;
  opt->use_direct_io_for_flush_and_compaction = true;

  // Ensure statistics are enabled so we can see the Hit Ratio
  if (opt->statistics == nullptr) {
      std::cerr << "[JNI-FORCED] Statistics object was null, creating one..." << std::endl;
      opt->statistics = ROCKSDB_NAMESPACE::CreateDBStatistics();
  }
