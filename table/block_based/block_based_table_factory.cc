...
// extern std::string g_secondary_cache_full_uri;
extern std::string g_secondary_cache_id;
...
static std::shared_ptr<SecondaryCache> global_sec_cache;
...
void BlockBasedTableFactory::InitializeOptions() {
  ...
  if (table_options_.no_block_cache) {
    ...
    if (!g_secondary_cache_id.empty()) {

      if (!global_sec_cache) {
        std::shared_ptr<SecondaryCache> sec;

        // ONLY pass ID
        Status s = SecondaryCache::CreateFromString(
            ConfigOptions(),
            g_secondary_cache_id,
            &sec);

        if (!s.ok() || !sec) {
          std::cerr << "[ERROR] Failed to create SecondaryCache: "
                    << s.ToString() << std::endl;
          abort();
        }

        global_sec_cache = sec;

        std::cerr << "[INIT ONCE] SecondaryCache created: "
