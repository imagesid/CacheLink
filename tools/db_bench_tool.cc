...
#include "cachelink.h"
std::string g_secondary_cache_full_uri;
...
void RegisterMyCache() {
  rocksdb::ObjectLibrary::Default()->AddFactory<rocksdb::SecondaryCache>(
      ...
      const std::string& full_uri =
          g_secondary_cache_full_uri.empty() ? uri : g_secondary_cache_full_uri;
...
DEFINE_string(cachelink, "", "Extra options for RocksCachelibWrapper");
...
// Current the following isn't equivalent to OS_LINUX.
  ...
  static std::shared_ptr<Cache> NewCache(int64_t capacity) {
    ...
    if (!FLAGS_secondary_cache_uri.empty()) {
      ...
      g_secondary_cache_full_uri = FLAGS_secondary_cache_uri + "," + FLAGS_cachelink;
      Status s = SecondaryCache::CreateFromString(
          ConfigOptions(), FLAGS_secondary_cache_uri, &secondary_cache);
      if (secondary_cache == nullptr) {
        fprintf(stderr,
...
int db_bench_tool(int argc, char** argv) {
  RegisterMyCache();
  fprintf(stdout, "CacheLib integration: enabled\n");
