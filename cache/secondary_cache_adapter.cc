...
Cache::Handle* CacheWithSecondaryAdapter::Promote(
  ...
  if (result != nullptr) {
    RecordTick(stats, SECONDARY_CACHE_PROMOTIONS);
  }
  return result;
}
...
Cache::Handle* CacheWithSecondaryAdapter::Lookup(const Slice& key,
  ...
  if (!result && secondary_compatible) {
    ...
    if (secondary_handle) {
      ...
      // Secondary lookup returned a handle, but no usable object was promoted.
      if (!result) {
        RecordTick(stats, SECONDARY_CACHE_MISSES);
      }
    } else {
      // DRAM missed and secondary cache also missed.
      RecordTick(stats, SECONDARY_CACHE_MISSES);
    }
  }
  return result;
