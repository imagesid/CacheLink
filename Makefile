...
EXTRA_CXXFLAGS += \
	...
	-I$(CACHELIB_PATH)/include

LDFLAGS += -L$(CACHELIB_PATH)/lib
...
# 	$(CXX) $^ -o $@ \
# 	-L. -lrocksdb \
# 	-Wl,--whole-archive $(WRAPPER_PATH)/libRocksCachelibWrapper.a -Wl,--no-whole-archive \
# 	-Wl,--start-group \
# 	-L$(CACHELIB_PATH)/lib \
# 	$(CACHELIB_LIBS) \
# 	-Wl,--end-group \
# 	$(SYSTEM_LIBS) \
# 	-Wl,-rpath,. \
# 	-Wl,-rpath,$(CACHELIB_PATH)/lib
# Define wrapper objects
# Fix CacheLib build issues ONLY for cache.cc
$(OBJ_DIR)/java/rocksjni/cache.o: CXXFLAGS += \
    -Wno-unknown-pragmas \
    -Wno-shadow \
    -Wno-error \
    -frtti

$(OBJ_DIR)/tools/cachelink.o: CXXFLAGS += \
    -Wno-unknown-pragmas \
    -Wno-shadow \
    -Wno-error \
    -frtti
# $(OBJ_DIR)/tools/cachelink.o: CXXFLAGS += -Wno-unknown-pragmas -Wno-shadow
$(OBJ_DIR)/tools/db_bench_tool.o: CXXFLAGS += -Wno-unknown-pragmas -Wno-shadow
db_bench: $(OBJ_DIR)/tools/db_bench.o \
          $(BENCH_OBJECTS) $(TESTUTIL) $(LIBRARY)
	$(CXX) $^ -o $@ \
	-L. -lrocksdb \
	-Wl,--start-group \
	-L$(CACHELIB_PATH)/lib \
...
rocksdbjavastatic_javalib: 
	cd java; $(MAKE) javalib
	rm -f java/target/$(ROCKSDBJNILIB)
	$(CXX) $(CXXFLAGS) -frtti -fexceptions \
	-Wno-error=unknown-pragmas -Wno-error=shadow \
	-I./java/. $(JAVA_INCLUDE) -shared -fPIC \
	-o ./java/target/$(ROCKSDBJNILIB) $(ALL_JNI_NATIVE_SOURCES) \
	$(LIB_OBJECTS) $(COVERAGEFLAGS) \
	$(JAVA_COMPRESSIONS) $(JAVA_STATIC_LDFLAGS) \
	-L. \
